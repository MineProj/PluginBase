...
public final class PathUtils {
    ...
    public static Path copyFileToDirectory(final URL sourceFile, final Path targetDirectory, final CopyOption... copyOptions) throws IOException {
        final Path resolve = targetDirectory.resolve(org.apache.commons.io.FilenameUtils.getName(sourceFile.getFile()));
     ...
     * Compares the file contents of two Paths to determine if they are equal or not.
     * <p>
     * File content is accessed through {@link org.apache.commons.io.RandomAccessFileMode#create(Path)}.
     * </p>
     *
     * @param path1       the first stream.
     * @param path2       the second stream.
    ...
    public static boolean fileContentEquals(final Path path1, final Path path2, final LinkOption[] linkOptions, final OpenOption[] openOptions)
        ...
        // Faster:
        try (RandomAccessFile raf1 = org.apache.commons.io.RandomAccessFileMode.READ_ONLY.create(path1.toRealPath(linkOptions));
             RandomAccessFile raf2 = org.apache.commons.io.RandomAccessFileMode.READ_ONLY.create(path2.toRealPath(linkOptions))) {
            ...
            try (InputStream inputStream1 = Files.newInputStream(nPath1, openOptions);
                ...
                return org.apache.commons.io.IOUtils.contentEquals(inputStream1, inputStream2);
            }
        }
    }
    ...
    public static Path getTempDirectory() {
        return Paths.get(org.apache.commons.io.FileUtils.getTempDirectoryPath());
    }
    ...
    public static String readString(final Path path, final Charset charset) throws IOException {
        return new String(Files.readAllBytes(path), org.apache.commons.io.Charsets.toCharset(charset));
    }
    ...
    public static boolean waitFor(final Path file, final Duration timeout, final LinkOption... options) {
        ...
        try {
            while (!exists(file, options)) {
                ...
                try {
                    org.apache.commons.io.ThreadUtils.sleep(Duration.ofMillis(Math.min(minSleepMillis, finishInstant.minusMillis(now.toEpochMilli()).toEpochMilli())));
    ...
    public static Path writeString(final Path path, final CharSequence charSequence, final Charset charset, final OpenOption... openOptions)
        ...
        Files.write(path, String.valueOf(charSequence).getBytes(org.apache.commons.io.Charsets.toCharset(charset)), openOptions);
        return path;
    }
