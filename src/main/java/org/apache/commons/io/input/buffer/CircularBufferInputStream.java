...
public class CircularBufferInputStream extends FilterInputStream {
     ...
     * ({@link org.apache.commons.io.IOUtils#DEFAULT_BUFFER_SIZE}).
     *
     * @param inputStream The input stream, which is being buffered.
     */
    public CircularBufferInputStream(final InputStream inputStream) {
        this(inputStream, org.apache.commons.io.IOUtils.DEFAULT_BUFFER_SIZE);
    }
    ...
    protected void fillBuffer() throws IOException {
        ...
        final byte[] buf = org.apache.commons.io.IOUtils.byteArray(space);
        while (space > 0) {
            final int res = in.read(buf, 0, space);
            if (res == org.apache.commons.io.IOUtils.EOF) {
                eof = true;
                return;
            }
            if (res > 0) {
